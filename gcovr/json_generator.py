...
def print_json_report(covdata, output_file, options):
    ...
    if options.prettyjson:
        write_json = functools.partial(
            write_json, indent=PRETTY_JSON_INDENT, separators=(',', ': '), sort_keys=True)
    else:
...
def _json_from_branches(branches):
    json_branches = [_json_from_branch(branches[no])
                     for no in sorted(branches)]
